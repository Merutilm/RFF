...
public class GLShader {
    ...
    public GLShader(String vertexName, String fragName) {
        ...
        try {
            ...
            File vertexFile = new File(IOUtilities.getResource(vertexName).toURI());
            String vertexSrc = new String(Files.readAllBytes(vertexFile.toPath()));

            File fragFile = new File(IOUtilities.getResource(fragName).toURI());
            String fragSrc = new String(Files.readAllBytes(fragFile.toPath()));
    ...
    public synchronized void resetFBOTexture(int panelWidth, int panelHeight) {
        ...
        fboTextureID = recreateTexture2D(fboTextureID, panelWidth, panelHeight, GLShader.TextureFormat.FLOAT4);
