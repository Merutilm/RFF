...
public final class VideoRenderWindow extends CSFrame{
    ...
    public static void createVideo(Settings settings, VideoSettings videoSettings, File dir, File out){
        double logZoomPerSecond = videoSettings.logZoomPerSecond();
        double fps = videoSettings.fps();
        double frameZoomingPerSecond = logZoomPerSecond / MathConstants.LOG2;
        double frameInterval = frameZoomingPerSecond / fps;
        
        RenderState state = new RenderState();
        int currentID = state.getId();
        RFFMap targetMap = RFFMap.readMapByID(dir, 1);
        if(targetMap == null){
        ...
        TaskManager.runTask(() -> {
            ...
            try(
                ...
                int maxNumber = RFFUtils.generateFileNameNumber(dir, RFFUtils.EXTENSION_MAP);
    ...
    private static RFFMap getFrame(RenderState state, int currentID, File dir, double frame, double multiplier) throws IllegalRenderStateException, InterruptedException{
        ...
        RFFMap im1 = RFFMap.readMapByID(dir, f1);
        RFFMap im2 = RFFMap.readMapByID(dir, f2);
        ...
        double zc = AdvancedMath.ratioDivide(z1, z2, r);
